(() => {
  ...
  async function fetchCensysSummary(silent = false) {
    ...
    try {
      ...
      const payload = qs('#apiPayload');
      if (payload && !silent) {
        payload.textContent = JSON.stringify({ error: err.message }, null, 2);
      }
  ...
  async function initAuth0() {
    ...
    if (!AppState.settings.auth0Domain || !AppState.settings.auth0ClientId) {
